...
- [快速上手](#快速上手)
  - [获取源码](#获取源码)
  - [获取数据集](#获取数据集)
    - [coco-2017数据集-detect/segment任务](#coco-2017数据集-detectsegment任务)
    - [coco-pose数据集-pose任务](#coco-pose数据集-pose任务)
    - [ImageNet数据集-classify任务](#imagenet数据集-classify任务)
    - [DOTAv1数据集-obb任务](#dotav1数据集-obb任务)
  - [获取权重](#获取权重)
  - [执行推理](#执行推理)
  - [性能数据](#性能数据)
...
## 执行推理
```bash
python3 infer.py --task=${task_name} --pth=${pth} --dataset=${dataset} --batchsize=${batch_size} --device=${device_id}
```
