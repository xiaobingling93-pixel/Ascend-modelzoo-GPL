...
class ClassificationValidator(BaseValidator):
    ...
    def gather_stats(self) -> None:
        ...
        if RANK == 0:
            ...
            # Flatten gathered predictions
            self.pred = []
            for rank in gathered_preds:
                for pred in rank:
                    self.pred.append(pred)

            # Flatten gathered targets
            self.targets = []
            for rank in gathered_targets:
                for target in rank:
...
def val(om_path: str, data_path: str, device_id: int = 0, batch_size: int = 8, imgsz: int = 224):
    """
    Evaluate classification accuracy on ImageNet-style validation set using OM model.

    Args:
        om_path (str): Path to the OM model file.
        data_path (str): Root directory of validation set, containing subfolders named by class.
        device_id (int): Ascend device ID.
        batch_size (int): Batch size for inference.
        imgsz (int): Input image size (YOLOv8-cls typically 224).

    Returns:
        dict: Statistics dictionary containing top1 and top5 accuracy.
    """
    val_root = Path(data_path)
    if not val_root.exists():
        raise FileNotFoundError(f"Validation set path does not exist: {val_root}")

    # Automatically discover class names from subfolders
    class_names = sorted([d.name for d in val_root.iterdir() if d.is_dir()])
    if not class_names:
        raise RuntimeError(f"No class subfolders found under {val_root}. Ensure dataset structure is correct.")

    nc = len(class_names)
    LOGGER.info(f"Found {nc} classes, first class: {class_names[0]}")
    ...
    validator.data = data_path
    validator.names = {i: name for i, name in enumerate(class_names)}
    validator.nc = nc
    validator.metrics.names = validator.names

    dummy_model = SimpleNamespace(names=validator.names)
    validator.init_metrics(dummy_model)

    # Build dataloader
    dataloader = validator.get_dataloader(data_path, batch_size)
    ...
    for _, batch in enumerate(TQDM(dataloader, desc, n_batches)):
        batch_pre = validator.preprocess(batch)

        # Convert torch tensor to numpy for InferSession
        img_np = batch_pre["img"].numpy()

        try:
            outputs = model.infer([img_np])[0]
        except Exception as e:
            LOGGER.error(f"Inference failed: {e}")
            continue

        preds = torch.from_numpy(outputs).float()
        preds = validator.postprocess(preds)
        validator.update_metrics(preds, batch_pre)
        total_images += batch_pre["img"].size(0)
    ...
    LOGGER.info(f"Validation completed, top1_acc={validator.metrics.top1:.4f}, top5_acc={validator.metrics.top5:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate YOLOv8 classification accuracy on ImageNet-style val set using OM model")
    parser.add_argument('--om', type=str, required=True, help='Path to OM model file')
    parser.add_argument('--data', type=str, required=True, help='Root directory of validation set (with class subfolders)')
    parser.add_argument('--batch_size', type=int, default=8, help='Batch size for inference')
    parser.add_argument('--device_id', type=int, default=0, help='Ascend device ID')
    parser.add_argument('--imgsz', type=int, default=224, help='Input image size (classification model usually 224)')
